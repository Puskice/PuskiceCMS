@section('content')
	<div class="row">
		<form method="post" action="{{URL::action('CommentController@postStore')}}">
			<div class="col-md-8">
				...
				<div class="box box-primary">
					...
					<div class="box-body">
		                ...
		                <div class="form-group">
		                    ...
		                    @if($news->postType == 1)
		                    	<p><strong><a href="{{_l(URL::action('NewsController@getEdit').'/'.$news->id)}}">{{$news->title}}</a></strong></p>
		                    @else
		                    	<p><strong><a href="{{_l(URL::action('PageController@getEdit').'/'.$news->id)}}">{{$news->title}}</a></strong></p>
		                    @endif
		                </div>
		            </div>
