        ...
        <div class="col-md-12">
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title">{{__(Lang::get('admin.allSubjects'))}}</h3>
                    <div class="box-tools">
                        ...
                        @foreach($news as $n)
	                        <tr>
	                            ...
	                            @if($n->deleted_at != null)
                                    <td><a class="btn btn-primary" href="{{_l(URL::action('SubjectController@getRestore').'/'.$n->id)}}">{{__(Lang::get('admin.restore'))}}</a></td>
                                @else
                                    <td><a class="btn btn-primary" href="{{_l(URL::action('SubjectController@getEdit').'/'.$n->id)}}">{{__(Lang::get('admin.edit'))}}</a></td>
                                @endif
                                @if($n->deleted_at != null)
                                    <td><a href="#" data-toggle="modal" data-target="#confirm-delete" class="btn btn-danger" data-href="{{URL::action('SubjectController@getDestroy')}}/{{$n->id}}">{{__(Lang::get('admin.delete'))}}</a></td>
                                @else
                                   <td><a href="#" data-toggle="modal" data-target="#confirm-delete" class="btn btn-danger" data-href="{{URL::action('SubjectController@getDelete')}}/{{$n->id}}">{{__(Lang::get('admin.delete'))}}</a></td>
                                @endif
	                        </tr>
                        @endforeach
                    </table>
