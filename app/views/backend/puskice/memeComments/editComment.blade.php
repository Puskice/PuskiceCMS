@section('content')
	<div class="row">
		<form method="post" action="{{URL::action('MemeCommentController@postUpdate')}}/{{$comment->id}}">
			<div class="col-md-8">
				...
				<div class="box box-primary">
					...
					<div class="box-body">
		                ...
		                <div class="form-group">
		                    ...
		                    <p><strong><a href="{{_l(URL::action('MemeController@getInstanceEdit').'/'.$comment->news_id)}}">{{$comment->meme->first_line}}</a></strong></p>
		                </div>
		            </div>
		            <div class="box-footer">
		            	<input type="hidden" value="{{Session::get('_token')}}" name="_token"/>
		            	<input type="hidden" value="{{$comment->meme->id}}" name="newsId"/>
