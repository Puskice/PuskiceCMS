@section('content')
	<div class="row">
		<form method="post" action="{{URL::action('NewsController@postUpdate')}}/{{$article->id}}">
			<div class="col-md-8">
				...
				<div class="box box-primary">
					...
					<div class="box-body">
						@if($article->published == 1)
							<p><strong>{{__(Lang::get('admin.viewNews'))}}: </strong><a target="_blank" href="{{_l(Request::root()."/vest/".dateToUrl($article->created_at)."/".$article->permalink)}}">{{$article->title}}</a> - {{__(Lang::get('admin.adminOnly'))}}</p> 
						@elseif($article->published == 2)
							<p><strong>{{__(Lang::get('admin.viewNews'))}}: </strong><a target="_blank" href="{{_l(Request::root()."/vest/".dateToUrl($article->created_at)."/".$article->permalink)}}">{{$article->title}}</a></p> 
						@endif
						<div class="form-group">
		                ...
		                <div class="form-group">
		                    ...
		                    <select class="form-control chosen-select" name="categories[]" multiple="multiple">
		                    	@foreach($categories as $category)
		                    		<?php $done = false; ?>
		                    		@foreach($article->newsCategories as $c)
		                ...
		                <div class="checkbox">
						    <label>
						      <input type="checkbox" name="featured" value="1" 
						      	@if($article->featured == 1) checked="checked" @endif> 
		            ...
		            <div class="box-footer">
                        ...
                        <a href="#" data-href="{{URL::action('NewsController@getDelete')}}/{{$article->id}}" class="btn btn-danger pull-right" data-toggle="modal" data-target="#confirm-delete">{{__(Lang::get('admin.delete'))}}</a>
                    </div>
                    <div class="box-footer">
